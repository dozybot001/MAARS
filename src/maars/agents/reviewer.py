"""Write / Reviewer — incremental structured-output academic peer review.

The Reviewer reports only a delta each round (resolved ids + new issues).
The system (reviewer_node in graphs/write.py) is responsible for applying
...
class ReviewFeedback(BaseModel):
    """Incremental feedback from one Reviewer round.

    The Reviewer only reports what changed this round — the system merges
    prior_issues with this feedback to obtain the new canonical list:
    ...
    resolved: list[str] = Field(
        ...
        description=(
            "IDs of prior issues that are now resolved in the current paper draft. "
...
def review_paper(
    ...
    *,
    ...
    """Run the Reviewer once on a paper draft and return incremental feedback.

    The caller (typically reviewer_node in the Write graph) is responsible
    for merging the feedback with prior_issues to obtain the new canonical
    list: next = (prior - resolved) + new_issues.
    """
    model = get_chat_model(temperature=0.0)
    reviewer = model.with_structured_output(ReviewFeedback)
