...
| RefineIdea | 生成 refined_idea（Markdown 字符串） |
...
- **Output**：WebSocket `idea-complete` 回传 `{ideaId, keywords, papers, refined_idea}`（refined_idea 为 Markdown 字符串），前端 `setTaskOutput('idea', ...)`，`setCurrentIdeaId(ideaId)`
