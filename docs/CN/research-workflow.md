...
Research 工作流由 7 个环节构成。Calibrate 仅在首次运行，其余环节在迭代中可重复执行。Evaluate 通过 `strategy_update` 字段决定是否触发下一轮 Strategy → Decompose → Execute 循环。

## 1. Calibrate — 校准原子任务粒度
...
**设计决策**：Strategy 接收能力画像和原子定义，确保推荐的方案在 agent 能力范围内。迭代中通过 Strategy Update 复用同一 prompt 更新策略，注入旧策略和评估反馈作为上下文。
...
| **输入** | 研究课题（或迭代上下文）+ atomic_definition + strategy（注入 system prompt） |
| **输出** | 扁平任务列表 + 树结构 |
...
在迭代轮次中，Decompose 接收 `_build_iteration_context` 构建的上下文（含已完成工作摘要和 artifacts），确保新任务不重复已有工作。
...
| `pass=false, redecompose=true` | 调用 `decompose(root_id=task_id)` 在原位将任务拆分为子任务 |

**设计决策**：
- Verify 不写 summary（由 Execute 负责），专注于验证产出是否存在
- Verify 被鼓励调用 `list_artifacts` 确认文件真实存在
...
- 输出包含 `strategy_update` → 触发 Strategy Update → Decompose · round N → Execute
- 不包含 `strategy_update` → 研究结束

**设计决策**：
- 传入历史评估，附标注"已尝试过——不要重复"
- 传入能力画像，让评估器知道建议的可行性
- 传入当前策略，让评估器判断策略本身是否需要调整

## 7. Strategy Update — 策略更新
...
复用 `STRATEGY_SYSTEM` prompt，通过 `build_strategy_update_user` 注入旧策略和评估上下文。更新后的策略驱动下一轮 Decompose，确保新任务沿调整后的方向推进。
