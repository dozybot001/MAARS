...
- Refine 阶段是真正的 multi-agent 协作（Agno Team coordinate 模式，agent 间有真实的信息流动）
- Write 阶段与 Refine 对称，使用 Agno Team coordinate 模式（Leader + Writer + Reviewer 协作）
...
1. **入口层**：Vue 3 前端 + FastAPI，用于启动、暂停、恢复和观察执行过程。
2. **编排层**：负责三阶段顺序和生命周期控制。
3. **阶段层**：每个 stage 是一个稳定边界，输入输出通过 session DB 连接。
4. **智能执行层**：Agno Team（multi-agent 协作）或 AgnoClient（单/多 client 编排）。
...
`AgentStage` 提供 `_stream_llm()` 方法——带重试、超时、SSE 事件广播的 LLM 流式调用。Research 使用此基类。

`TeamStage` 是 Refine 和 Write 的共享基类，提供通用的 Agno Team `run()` 执行循环和 `_handle_event()` 事件映射。子类只需定义 `_create_team()`、`_finalize()` 和两个配置属性（`_member_map`、`_capture_member`）。
...
Write 不是简单拼接，而是一个**全局一致性与叙事组织问题**。它使用 Agno Team coordinate 模式，与 Refine 完全对称：

```mermaid
flowchart LR
    IN["tasks/*.md + artifacts + refined_idea"] --> LEADER["Team Leader<br/>协调者"]
    LEADER -->|1. 委派| WRI["Writer Agent<br/>读取所有产出、写论文"]
    WRI -->|初稿| LEADER
...
Write 阶段是这一原则的典型应用：Writer 在写每个章节时通过工具按需读取相关 task 输出，而不是一次性把所有内容塞进 prompt。
...
前端不是调度者，而是 runtime 的观察器。SSE 使用 `fetch` + `ReadableStream` 实现，通过 `Authorization` header 认证。

### 5.4 Session 管理

完整的 session 生命周期：

- **列表**：`GET /api/sessions` — 所有历史 session
- **详情**：`GET /api/sessions/{id}` — session 元数据
- **状态恢复**：`GET /api/sessions/{id}/state` — 从文件推导完整前端状态，零额外持久化
- **删除**：`DELETE /api/sessions/{id}`

前端侧边栏可浏览历史 session，点击即可恢复完整 UI 状态（进度条、文档、任务树、日志回放）。
...
│   ├── orchestrator.py          #   三阶段顺序控制 + SSE 广播
│   ├── stage.py                 #   Stage + AgentStage 基类
│   ├── research.py              #   ResearchStage — agentic workflow 引擎
│   ├── decompose.py             #   递归任务分解 DAG
...
│
├── agno/                        # Agno 基础设施
│   ├── __init__.py              #   Stage factory（组装所有阶段）
│   ├── client.py                #   AgnoClient + StreamEvent
│   ├── models.py                #   Model factory（Google / Anthropic / OpenAI）
...
└── routes/                      # API 路由
    ├── pipeline.py              #   管道控制（start/stop/resume）
    ├── events.py                #   SSE 流式端点
    └── sessions.py              #   Session CRUD

frontend/src/
├── App.vue                      # 主应用（生命周期、SSE 连接）
├── api.js                       # API 调用封装（Bearer token 认证）
├── main.js                      # Vue 入口
├── stores/
│   └── pipeline.js              # Pinia 状态管理
├── composables/
│   ├── useSSE.js                # fetch + ReadableStream SSE
│   └── useAutoScroll.js         # 日志自动滚动
└── components/
    ├── ProgressBar.vue          # 7 阶段进度条 + 命令面板
    ├── LogViewer.vue            # 实时推理日志
    ├── ProcessViewer.vue        # 任务树 / 执行批次 / 文档
    ├── SessionDrawer.vue        # 会话侧边栏
    ├── DecompTree.vue           # 任务分解树
    ├── ExecList.vue             # 执行批次列表
    └── AppModal.vue             # 通用模态框
```
