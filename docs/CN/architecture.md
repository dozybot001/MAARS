...
flowchart LR
    ...
    subgraph loop ["while strategy_update"]
        direction TB
        STR["Strategy"] --> DEC["Decompose"] --> EXEC["Execute ⇄ Verify"] -->|all pass| EVAL{Evaluate}
        EVAL -->|strategy_update| STR
    end

    CAL --> STR
    EVAL -->|done| OUT["Output"]
```
...
```mermaid
flowchart TB
    TASK["Task Agent\nexecute + SUMMARY"] --> VER{Verify\nlist_artifacts}
    VER -->|pass| DONE["✓ 完成"]
    VER -->|fail| RETRY["Retry\n携带 review"] --> VER2{Verify}
    VER2 -->|pass| DONE
    VER2 -->|fail + redecompose| REDEC["decompose\n(root_id=task_id)"] --> TASK
    VER2 -->|fail| ERR["✗ 失败"]
```
