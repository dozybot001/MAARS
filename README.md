...
MAARS is a **hybrid multi-agent system**: Refine and Write use Agno Team coordinate mode (multi-agent collaboration), while Research uses a runtime-controlled agentic workflow with a direct Agno Agent. The three stages communicate only through the file-based session DB -- they are fully decoupled.

### Class Hierarchy

```
Stage                          -- lifecycle + unified SSE
|-- ResearchStage              -- agentic workflow (direct Agno Agent)
+-- TeamStage                  -- multi-agent (Agno Team coordinate)
    |-- RefineStage
    +-- WriteStage
```

`Stage` provides the unified lifecycle (start, pause, resume, stop) and a single SSE event format shared by all stages. `TeamStage` extends it with Agno Team coordinate semantics. `ResearchStage` extends it with a direct Agno Agent for the runtime-controlled workflow.
...
|---------|---------|-------------|
| `MAARS_GOOGLE_API_KEY` | -- | Google Gemini API key (required) |
| `MAARS_GOOGLE_MODEL` | `gemini-3-flash-preview` | Gemini model to use |
...
|-- log.jsonl                # Structured event log (all SSE events)
|-- execution_log.jsonl      # Docker execution records
|-- meta.json                # Session metadata and timing
...
The web UI provides real-time observability via a **unified SSE architecture**:

- **One event format** -- every stage emits the same structured SSE event shape, making the protocol consistent across Refine, Research, and Write
- **Three listeners** -- the frontend registers three SSE listeners (progress, reasoning, process) that each handle their slice of the unified event stream
- **Data-driven rendering** -- the UI is entirely driven by the event data; no stage-specific rendering logic, just generic components that react to the event payload
...
| Agent framework | Agno (Team coordinate + direct Agent) |
| LLM providers | Google Gemini (via Agno) |
...
| [SSE Architecture (CN)](docs/CN/SSE_REFACTOR.md) | Unified SSE event format and frontend listener design |
