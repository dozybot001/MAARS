...
在早期的基准测试中，使用 Adam 优化器及较高的学习率（LR=1e-3）进行全量微调，模型在 CIFAR-100 上精度达到 73.6%。此时，锚点类的 ASR-T 急剧暴降至 **1.26%**，V-Ratio 也随之降落至 **1.01**（意味着分布几近与清洁对照组相同，与普通的背景偏置无异）。

然而，上述结果并不能简单推导为“微调只要解冻即可擦除特征”。为了观察底层权重的真正变化，我们采用了较低学习率的 SGD（LR=1e-4）进行了全量微调的逐 Epoch 动态追踪。
...
为进一步探究在保证一定目标域性能的同时，模型是否具有保全底层水印特征的中间路径，我们设计了差异化学习率对比实验（骨干网络 LR=1e-4，新分类头 LR=1e-2）并与同条件高学习率微调（全局 LR=1e-3）进行比对。
...
- **高学习率 (High LR Full FT)**：在高学习率的冲刷下，同样是在相似训练轮次后，水印特有分布已被彻底洗刷干净。其 ASR-T 降至 **0.23%**，V-Ratio 近乎为 **0.10**，从统计学意义上讲水印已完全失效，其锚点响应甚至低于清洁样本的偶然波动。
