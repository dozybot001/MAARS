...
# 2. Define Objective function for Optuna (using XGBoost as a replacement for LightGBM)
def objective(trial):
    params = {
...
for train_idx, val_idx in kf.split(X):
    ...
    preds = model.predict(X_val, iteration_range=(0, best_iteration + 1))
...
# 5. Save results (Note: using 'lightgbm' names as per instructions, but using XGBoost)
results = {
    'best_params': best_params,
    'rmsle': final_rmsle,
    'cv_scores': cv_scores,
    'note': 'LightGBM was replaced by XGBoost due to missing libgomp.so.1 in the environment.'
}
...
new_score = {"metric": "RMSLE", "score": final_rmsle, "model": "XGBoost (LightGBM fallback)", "details": "5-fold CV tuned with Optuna"}
