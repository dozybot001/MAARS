...
study.optimize(objective, n_trials=30) # Reduced trials for speed, but enough for a good result
...
for train_index, val_index in kf.split(X):
    ...
    # After fit, the model will use the best iteration automatically if early stopping is used
    preds = model.predict(X_val)
...
score_data = {"metric": "RMSLE", "score": float(final_rmsle), "model": "XGBoost", "details": "5-fold CV with Optuna tuning"}
