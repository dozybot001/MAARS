                    ...
                    async with asyncio.timeout(timeout):
                        async for event in agent.arun(user_text, stream=True, stream_events=True):
                            ...
                            if event.event == RunEvent.run_content:
                                ...
                                if event.metrics and self.db:
                                    meta = self.db.get_meta()
                                    self.db.update_meta(
                                        tokens_input=meta.get("tokens_input", 0) + (event.metrics.input_tokens or 0),
                                        tokens_output=meta.get("tokens_output", 0) + (event.metrics.output_tokens or 0),
                                        tokens_total=meta.get("tokens_total", 0) + (event.metrics.total_tokens or 0),
                                    )
                    return result
                except asyncio.CancelledError:
                    raise
