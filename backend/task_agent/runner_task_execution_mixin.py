...
class RunnerTaskExecutionMixin:
    ...
    async def _phase_execute(
        ...
        try:
            ...
            await self._append_step_event(task["task_id"], "task-execution-error", {"error": error_msg})
    ...
    async def _phase_validate(
        ...
        self._deps.set_worker_status(task_id, "validating")
        self._broadcast_worker_states()
        self._update_task_status(task_id, "validating")
        ...
        if use_mock:
            for chunk in self._deps.chunk_string(report, 20):
                ...
                await asyncio.sleep(self._deps.MOCK_VALIDATOR_CHUNK_DELAY)
            self._emit("task-step-b", {
                "taskId": task_id,
                "attempt": self._get_failure_count(task_id, "retry") + 1,
                "phase": "validation",
        ...
        if self.idea_id and self.plan_id:
            await self._deps.save_validation_report(
                self.idea_id, self.plan_id, task_id,
    ...
    async def _phase_finalize_success(
        self,
        task: Dict,
        run_attempt: int,
        report: str,
        ...
        await self._append_step_event(task_id, "task-completed", {
            "attempt": run_attempt,
            "validated": True,
            "validationSummary": validation_summary,
            "status": "done",
        ...
        logger.info("Task complete task_id={} completed={} remaining_pending={}", task_id, len(self.completed_tasks), len(self.pending_tasks))

        # Schedule downstream dependents
        dependents = self.reverse_dependency_index.get(task_id, [])
        ...
        for tid in self.pending_tasks:
            if tid not in self.running_tasks and tid not in self.completed_tasks:
                pt = self.task_map.get(tid)
    ...
    # ------------------------------------------------------------------
    # Orchestrator: _execute_task (was 490 lines, now ~60)
    # ------------------------------------------------------------------
    ...
    async def _execute_task(self, task: Dict) -> None:
        ...
        logger.info(
            ...
            len(self.running_tasks), len(self.completed_tasks), len(self.pending_tasks),
        )
        ...
        if reserved_attempt != run_attempt:
            ...
            self.task_next_attempt_hint[task_id] = max(
                int(self.task_next_attempt_hint.get(task_id) or 0), run_attempt,
            )
        if run_attempt > 1:
            self.task_phase_failure_count[self._failure_key(task_id, "retry")] = max(
                self._get_failure_count(task_id, "retry"), run_attempt - 1,
            )
        ...
        await self._append_step_event(task_id, "task-started", {
            "attempt": run_attempt,
            "description": task.get("description", ""),
            "dependencies": task.get("dependencies") or [],
        })

        on_thinking = self._make_on_thinking_callback(task, run_attempt)

        try:
            # Phase 2: Execute
            exec_passed, result, resolved_inputs, exec_error = await self._phase_execute(task, run_attempt, on_thinking)
            if not exec_passed:
                self._update_task_status(task_id, "execution-failed")
            ...
            # Phase 3: Validate
            val_passed, report, step_b_review, val_summary = await self._phase_validate(
                task, result, resolved_inputs, run_attempt, on_thinking,
            )
            if not val_passed:
                self._update_task_status(task_id, "validation-failed")
                await self._retry_or_fail(
            ...
            # Phase 4: Reflect
            await self._reflect_on_task(task, result, on_thinking)

        except Exception as e:
            async with self._worker_lock:
                self._deps.release_worker(task_id)
            self._broadcast_worker_states()
            await self._append_step_event(task_id, "task-error", {"error": str(e)})
            raise
        finally:
            task_container_name = self.task_docker_containers.pop(task_id, "")
            if task_container_name:
                try:
        ...
        await self._phase_finalize_success(task, run_attempt, report, val_summary)
