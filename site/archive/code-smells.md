...
> 2026-03-28 更新，同步 agno-only 架构精简

## MEDIUM — 有空修

| # | 问题 | 位置 | 说明 |
|---|------|------|------|
| 13 | Prompt 模板散落在各 stage 文件顶部 | `research.py` | `_AUTO` 前缀可提取到 `pipeline/prompts.py` |
...
| 原 # | 问题 | 修复方式 |
|------|------|---------|
| 11 | WriteStage phase 用字符串而非 Enum | WriteStage 已删除（agno-only 用 AgentWriteStage） |
| 12 | `has_broadcast` leaky abstraction | 已删除，Client 不再持有 broadcast 引用 |
| 14 | ADK 内置工具 import 失败静默吞掉 | ADK 模式已删除 |
| 16 | Task dict vs dataclass 混用 | `decompose.py` 内部用 `Task` dataclass，对外统一输出 `dict` |
| 23 | `max_rounds=999` hack（WriteStage） | WriteStage 已删除 |
| 24 | MockClient `_task_counters` 不清理 | Mock 模式已删除 |
| A2(旧) | Pipeline ↔ Agent 双向依赖 | `generate_reproduce_files` 已移至 `reproduce.py` |
| A3(旧) | Plan/Execute 都 override run() | 合并为 `ResearchStage` |

## 修复建议优先序

1. **HIGH**: 全局状态 → FastAPI DI + per-session orchestrator
2. **HIGH**: 日志/可观测性 → 接入 Python logging + `/health` endpoint
3. **MEDIUM**: `_AUTO` 提取到 `pipeline/prompts.py`
